...
def get_serie(parser, token):
    ...
    Usage::
        {% get_serie for <publishable>[ limit <X>] as <result> %}

    Examples::

        {% get_serie for object as serie %}
        {% get_serie for object limit 10 as serie %}
    """
...
class SerieNode(template.Node):
    ...
    def render(self, context):
        obj = template.Variable(self.params['for']).resolve(context)

        if isinstance(obj.target, Serie):
            ...
            try:
                current_part = SeriePart.objects.get_for_object(obj)
...
def serie_navigation(obj):
    ...
    try:
        part = SeriePart.objects.get_for_object(obj)
    except SeriePart.DoesNotExist:
        return out
